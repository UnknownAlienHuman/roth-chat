...
- [x] Return only `false` on no-op paths so Blizzard retains the current secure tuple; return the complete 19-field replacement only after a real `arg1` change.
- [x] Preserve every routing, sender, line-ID, access-ID, and metadata field unchanged whenever text is replaced.
...
- [x] Stop forwarding trailing `AddMessage` varargs into addon modules; expose only accessible rendered text and numeric color primitives.
...
- [x] Review confirms the dispatcher signature and replacement path contain all 19 arguments.
...
- [x] Review confirms Roth Chat uses that no-op path and repacks the tuple only after visible text changes.
